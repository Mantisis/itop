		...
		if ($iMsgCount > 0)
		{
			...
			for($iMessage = $iStart; $iMessage < $iStart+$iMsgCount; $iMessage++)
			{
				...
				if (array_key_exists($sUIDLs, $aProcessed))
				{
					...
					{
						$oTicket = MetaModel::GetObject('Ticket', $aProcessed[$sUIDLs]['ticket_id']);
						if($oTicket != null) {
							$sTicketClass = get_class($oTicket);
						}
						else {
							$sTicketClass = $oInbox->Get('target_class');
						}

						$sTicketUrl = ApplicationContext::MakeObjectUrl($sTicketClass, $aProcessed[$sUIDLs]['ticket_id']);
						$sLink = '<a href="'.$sTicketUrl.'">'.$sTicketClass.'::'.$aProcessed[$sUIDLs]['ticket_id'].'</a>';
					}
				}
